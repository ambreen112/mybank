...
class Bank {
    ...
    transactions: { accNumber: number; type: string; amount: number}[] = [];
...
addTransaction(accNumber: number, type: string, amount: number) {
    this.transactions.push({ accNumber, type, amount });
}

// Method to get transactions for a specific account
getTransactions(accNumber: number): { type: string; amount: number }[] {
    return this.transactions.filter(
        (transaction) => transaction.accNumber === accNumber
    );
}
...
async function bankservice(bank:Bank) {
    ...
    if (service.select == "View Balance"){
        ...
        if(!account){
            ...
            let customer = myBank.customers.find(
